...
namespace Hellsinki2017
{
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Console.WriteLine($" versenyzők száma: {rovidprogram.Count()}");
            ...
            try
            {
                Console.WriteLine("A vers pontszama: " + ÖsszPontszam(input));
            }
            catch (Exception e)
            {
            ...
            {
                Console.WriteLine($"{item.Key}: {item.Value} vers");
            }
        ...
        static float ÖsszPontszam(string name)
        {
            ...
            if (inst is null) throw new Exception("nincs ilyen");
            if (dontoInst != null) sum += dontoInst.Technikai + dontoInst.Komponens - dontoInst.Hibabont;
